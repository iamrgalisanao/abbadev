...
- It contains the **authoritative `EVENTS` catalog** (title, schedule, price, location). An unknown `eventId` makes it throw.
...
- the n8n `EVENTS` catalog and `eventOfferings` / `flagshipSeminar` in `App.jsx`
