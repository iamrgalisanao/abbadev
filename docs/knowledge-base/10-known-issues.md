...
| 14 | **Fixed 2026-09-25** (the flagship only shows until it starts; `/seminar` otherwise features the next upcoming session). The flagship seminar (Sep 5, 2026) is past, but it is still the default `/seminar` offer | `flagshipSeminar` in `App.jsx` |
...
| 21 | **Fixed 2026-09-25** (`kicker` is now a per-case `workflow` field; default "Live pipeline"). `CaseWorkflow` kicker is hard-coded to "n8n pipeline", even on the CRM, Stockora and transaction cases | `src/CaseWorkflow.jsx` |
